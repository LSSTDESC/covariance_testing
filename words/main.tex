...
\begin{document}
	...
	We run the likelihood analysis using only one KL-mode, meaning that we compress the 10 tomographic combinations for each angle into one number.  This data vector has a length of 19 data points by the 10-to-1 tomographic compression. And we show the constraints on the $\Omega_m - \sigma_8$ plane in \figref{kl-constaints}. We can see that after the compression, the cosmological constraints gets broader. This confirms the fact that the first KL-mode does not contain all the information in the data.
